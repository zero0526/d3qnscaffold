...
class TD3ReplayBuffer:
    def __init__(self, max_size, node_type, state_dim, node_dim, mf_dim, device="cpu"):
        ...
        # Actions are probabilities in Simplex TD3
    ...
    def add(self, prev_state, prev_mf, action, reward, curr_state, curr_mf, done, agent_id=0, action_mask=None):
        ...
        self.action[self.ptr] = self._to_tensor(action, torch.float32)
        
        if action_mask is not None:
            self.action_mask[self.ptr] = self._to_tensor(action_mask, torch.float32)
        else:
            self.action_mask[self.ptr] = 1.0
    ...
    def add_batch(self, prev_states, prev_mfs, actions, rewards, curr_states, curr_mfs, dones, agent_ids, action_masks=None):
        ...
        self.action[indices] = actions.detach().to(device=self.device, dtype=torch.float32)

        if action_masks is not None:
            self.action_mask[indices] = action_masks.detach().to(device=self.device, dtype=torch.float32)
        else:
            self.action_mask[indices] = 1.0
    ...
    def sample(self, batch_size):
        ...
        return (
            ...
            self.prev_mf[ind],
            self.action[ind],
            self.action_mask[ind],
            self.reward[ind],
            self.curr_state[ind],
            self.curr_mf[ind],
            self.done[ind],
            self.agent_id[ind].squeeze(-1)
        )

    def __len__(self):
        return self.size

class MultiAgentTD3ReplayBuffer:
    def __init__(self, num_agents, node_type, max_size_per_agent, state_dim, node_dim, mf_dim, device="cpu"):
        self.num_agents = num_agents
        self.device = device
        self.node_type = node_type
        self.buffers = [
            TD3ReplayBuffer(max_size_per_agent, node_type, state_dim, node_dim, mf_dim, device)
    ...
    def get_len(self, agent_id=None):
        if agent_id is None: return self.total_size
