...
class SACReplayBuffer:
    def __init__(self, max_size, node_type, state_dim, node_dim, device="cpu"):
        ...
        # state=  backlog_s + f-allo_s + workload + placement_s
    ...
    def add(self, state, action, reward, next_state, done, agent_id=0):
...
class MultiAgentSACReplayBuffer:
    def __init__(self, num_agents, node_type, max_size_per_agent, state_dim,node_dim, device="cpu"):
        self.num_agents = num_agents
        self.device = device
        self.node_type = node_type
        self.buffers = [
            SACReplayBuffer(max_size_per_agent, node_type, state_dim, node_dim, device)
    ...
    def add_batch(self, states, actions, rewards, next_states, dones, agent_ids):
        # Maintain everything in tensor form
        a_ids = agent_ids.view(-1)
        for i in range(len(a_ids)):
            a_id = int(a_ids[i])
            self.buffers[a_id].add(
                states[i], actions[i], rewards[i], next_states[i], dones[i], a_id
            )
