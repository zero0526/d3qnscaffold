...
class FedRepCritic(nn.Module):
    """
    Critic implementing Federated Representation Learning.
    Shared Encoder across all instances, but Local Value Heads for personalization.
    """
    def __init__(self, state_dim, mf_dim, hidden_sizes, num_instances):
        super().__init__()
        h1, h2 = hidden_sizes
        
        # 1. Global Shared Encoder (Standard Linear, not MultiInstance)
        ...
        # 2. Branch Norm (Interface before local heads)
        self.branch_norm = nn.LayerNorm(h2)
        
        # 3. Local Value Heads (Instance-specific)
        self.value_head = MultiInstanceLinear(num_instances, h2, 1)

    def forward(self, state, mf, agent_indices):
        x = torch.cat([state, mf], dim=-1)
        latent = self.encoder(x)
        latent = self.branch_norm(latent)
        value = self.value_head(latent, agent_indices)
...
class PPOSCAFFOLDREPAgent:
    def __init__(self, node_id, node_type, state_dim, action_dim, u_action_dim,
                 ...
                 target_entropy_ratio=0.8, target_entropy_end_ratio=0.05,
        ...
        self.optimizer_critic_head = optim.Adam(self.critic.value_head.parameters(), lr=lr)
        self.optimizer_critic_encoder = optim.Adam(
            list(self.critic.encoder.parameters()) + list(self.critic.branch_norm.parameters()), 
            lr=lr
        )
        ...
        # SCAFFOLD Control Variates for Critic Encoder representation
        self.shared_params = list(self.critic.encoder.parameters()) + list(self.critic.branch_norm.parameters())
        self.critic_c = [torch.zeros_like(p, device=self.device) for p in self.shared_params]
        self.critic_ci = [
            [torch.zeros_like(p, device=self.device) for p in self.shared_params]
    ...
    def _local_update(self, b_states, b_pred_mfs, b_actions, b_old_log_probs, b_advantages, b_returns, b_agent_ids, b_masks, zeta=1.0):
        """PHASE 1: Local update for Actor and Critic Head. Freeze Shared Encoder."""
        ...
        log_probs, entropy = self.actor.evaluate(
            b_states, b_pred_mfs, b_actions, masks=b_masks, indices=b_agent_ids, exclude_zero=self.exclude_zero,
            zeta=zeta
        )
        ...
        # --- 2. Critic Head Update ---
        vals = self.critic(b_states, b_pred_mfs, agent_indices=b_agent_ids)
        critic_head_loss = F.mse_loss(vals, b_returns)
        
        self.optimizer_critic_head.zero_grad()
        critic_head_loss.backward()
        self.optimizer_critic_head.step()

        return actor_loss.item(), critic_head_loss.item()

    def _scaffold_global_update(self, b_states, b_pred_mfs, b_returns, b_agent_ids, b_group_ids):
        """PHASE 2: Global update Shared Encoder with SCAFFOLD. Freeze Head."""
        # Unfreeze Shared Encoder
        for p in self.shared_params: 
            p.requires_grad = True

        u_groups = torch.unique(b_group_ids)
        group_grads = {g.item(): [] for g in u_groups}

        # --- 1. Calculate gradient per group (Client) ---
        for g in u_groups:
            g_item = g.item()
            g_mask = (b_group_ids == g)
            if g_mask.sum() == 0: continue
            
            # Forward pass for group g
            v_g = self.critic(b_states[g_mask], b_pred_mfs[g_mask], agent_indices=b_agent_ids[g_mask])
            loss_g = F.mse_loss(v_g, b_returns[g_mask])
            
            # Use autograd.grad to get gradient directly
            grads_g = torch.autograd.grad(loss_g, self.shared_params, retain_graph=False)
            
            # Update Local Control Variate (c_i) for group g
            for i, grad_i in enumerate(grads_g):
                self.critic_ci[g_item][i] = (
                    self.scaffold_beta * self.critic_ci[g_item][i] + 
                    (1 - self.scaffold_beta) * (grad_i.clone() - self.critic_c[i])
                )
                group_grads[g_item].append(grad_i.clone())

        # --- 2. Aggregate and Correct Gradient (SCAFFOLD Correction) ---
        self.optimizer_critic_encoder.zero_grad()
        corrected_grads = []
        
        for i, p in enumerate(self.shared_params):
            avg_g = torch.zeros_like(p)
            avg_ci = torch.zeros_like(p)
            
            for g_item in group_grads.keys():
                avg_g += group_grads[g_item][i]
                avg_ci += self.critic_ci[g_item][i]
                
            avg_g /= len(group_grads)
            avg_ci /= len(group_grads)
            
            # SCAFFOLD: corrected_grad = avg_grad + c_server - avg_c_client
            c_grad = avg_g + self.critic_c[i] - avg_ci
            corrected_grads.append(c_grad)
            
            # Update Server Control Variate (c)
            self.critic_c[i] = avg_ci.clone()

        # --- 3. Assign corrected gradient and Step Optimizer ---
        for i, p in enumerate(self.shared_params):
            if p.grad is None:
                p.grad = corrected_grads[i]
            else:
                p.grad.copy_(corrected_grads[i])
                
        torch.nn.utils.clip_grad_norm_(self.shared_params, max_norm=1.0)
        self.optimizer_critic_encoder.step()

    def learn(self, agents_ids: torch.Tensor = None, group_ids: torch.Tensor = None, zeta=1.0):
        ...
        for _ in range(self.k_epochs):
            ...
            for start in range(0, dataset_size, self.batch_size):
                ...
                # Re-predict MF for mini-batch (detach to avoid affecting MF net)
                b_pred_mfs = self.mf_net(torch.cat([b_states, b_prev_mfs], dim=-1), group_indices=b_group_ids).detach()

                # --- PHASE 1 ---
                _, critic_head_loss = self._local_update(
                    b_states, b_pred_mfs, b_actions, b_old_log_probs, b_advantages, b_returns, b_agent_ids, b_masks, zeta=zeta
                )

                # --- PHASE 2 ---
                self._scaffold_global_update(
                    b_states, b_pred_mfs, b_returns, b_agent_ids, b_group_ids
                )
                
                total_v_loss += critic_head_loss
                total_batches += 1

        # 3. Entropy Auto-tuning (SAC-style)
        with torch.no_grad():
            ...
            # Unfreeze shared params for entropy evaluation
            for p in self.shared_params: p.requires_grad = True
    ...
    def save(self, path):
        checkpoint = {
            ...
            'critic_encoder': self.critic.encoder.state_dict(),
            'critic_head': self.critic.value_head.state_dict(),
            'mf_net': self.mf_net.state_dict(),
            'critic_c': self.critic_c,
            'critic_ci': self.critic_ci,
    ...
    def load(self, path):
        ...
        self.critic.encoder.load_state_dict(checkpoint['critic_encoder'])
        self.critic.value_head.load_state_dict(checkpoint['critic_head'])
        self.mf_net.load_state_dict(checkpoint['mf_net'])
        self.critic_c = checkpoint['critic_c']
        self.critic_ci = checkpoint['critic_ci']
        self.learn_step_counter = checkpoint.get('learn_step', 0)

    def set_lr_factor(self, factor):
        for opt in [self.optimizer_actor, self.optimizer_critic_head, self.optimizer_critic_encoder, self.mf_optimizer]:
