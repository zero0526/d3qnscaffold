...
def deplete_float_queue(backlog, deadline, terminal_queue, age_queue, src_node_mapping, cpu_alloc, slot_duration):
    ...
    Trả về realized_delays cho các task hoàn thành trong slot.
    """
    ...
    # 5. Xử lý task đang dở dang (pending) - Gọt bớt workload
...
def age_and_clean_dual_queue(backlog, deadline, slot_duration, *aux_queues):
    ...
    # 2. Xác định các task vi phạm (Deadline <= 0)
    ...
    # 3. Lấy thông tin các task bị fail (terminal_id, svc_id)
    failed_terminal_ids = None
    failed_svc_ids = None
    if len(aux_queues) > 0:
        term_queue = aux_queues[-1] # Giả định terminal_queue luôn là aux cuối cùng
    ...
    # 5. Xóa Task vi phạm và làm sạch
